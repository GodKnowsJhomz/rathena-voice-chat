...
static bool session_matches(const ClientSession* s, const json& j) {
	...
	uint64_t sid = 0;
	try { sid = j.value("session_id", static_cast<uint64_t>(0)); }
	catch (...) { sid = 0; }
...
if (type == "auth") {
	...
	if (s->awaiting_advisory && s->char_id != 0 &&
		(s->char_id != new_cid || s->session_id != new_sid)) {
		const int old_cid = s->char_id;
		const uint64_t old_sid = s->session_id;
		{
			std::lock_guard<std::shared_mutex> lock(g_session_mtx);
			auto it_old = g_by_char_id.find(old_cid);
			if (it_old != g_by_char_id.end() && it_old->second == s)
				g_by_char_id.erase(it_old);
		}
		s->awaiting_advisory = false;
		s->advisory_wait_tick = 0;
		LOG_INFO("pending auth superseded  old_char_id=%d old_sid=%llu new_char_id=%d new_sid=%llu",
				 old_cid,
				 static_cast<unsigned long long>(old_sid),
				 new_cid,
				 static_cast<unsigned long long>(new_sid));
	}
...
int online_snapshot = 0;
{
 ...
 old->authed  = false;
 old->kicking = true;
 // Remove old from ALL indexes NOW, while it is still definitely valid.
 // (Previously idx_remove(old) ran AFTER old->ws->end() below — if the
 // transport tore the session down during end(), that idx_remove touched
 // freed memory and corrupted g_by_map / the spatial index, which later
 // crashed the UDP position loop in idx_set_position on a dangling entry.)
 idx_remove(old);
 // Close the old socket on the loop thread (never inline here): re-look-up by
 // ws pointer under the lock so a session freed meanwhile is a harmless miss.
 if (g_voice_loop.load() && old->ws) {
	 void*    old_ws  = old->ws;
	 uint64_t old_sid = old->session_id;
	 g_voice_loop.load()->defer([old_ws, old_sid]() {
		 std::lock_guard<std::shared_mutex> lock(g_session_mtx);
		 auto it = g_by_ws.find(old_ws);
		 if (it == g_by_ws.end() || !it->second) return;
		 ClientSession* o = it->second;
		 if (o->session_id != old_sid || !o->ws) return;
		 o->ws->send(json{{"type","error"},{"message","session replaced by new login"}}.dump(),
					 VoiceTcp::OpCode::TEXT);
		 o->ws->end(1000, "replaced");
	 });
 }
}
}
